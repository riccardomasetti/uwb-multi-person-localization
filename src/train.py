...
complex_conv_filters = [32, 64, 128]
amplitude_conv_filters = [16, 32, 64]
conv_kernels = [3, 3, 3]
...
def encoder_pool(x, name):
    topk = TopKAveragePooling2D(ENCODER_TOPK, name=f"{name}_topk")(x)
    attn = SoftAttentionPooling2D(
        ENCODER_ATTN_TEMPERATURE, name=f"{name}_soft_attn"
    )(x)
...
x = tf.keras.layers.Lambda(
    lambda t: tf.reshape(t, [-1, n_bins_kept, n_freq_kept, N_ANTENNAS * n_channels])
)(x)
...
complex_token_dim = complex_last_conv_dim * 2
amplitude_token_dim = amplitude_last_conv_dim * 2

x_cplx = encoder_pool(x_cplx, "complex")
x_cplx = tf.keras.layers.Lambda(
    lambda t: tf.reshape(t, [-1, N_RADARS, complex_token_dim])
)(x_cplx)
x_amp = encoder_pool(x_amp, "amplitude")
x_amp = tf.keras.layers.Lambda(
    lambda t: tf.reshape(t, [-1, N_RADARS, amplitude_token_dim])
)(x_amp)
...
val_features_raw = val_features.copy()
test_features_raw = test_features.copy()

feat_mean_full = train_features_raw.mean(axis=0).astype(np.float32)
feat_std_full = train_features_raw.std(axis=0).astype(np.float32)
feat_std_full[feat_std_full == 0] = 1.0


def _build_normalization_config(mean_full, std_full):
    denominator_full = (std_full + STD_EPSILON).astype(np.float32)
    stats = {
        "mean": mean_full.mean(axis=0, keepdims=True).astype(np.float16),
        "denominator": denominator_full.mean(axis=0, keepdims=True).astype(
            np.float16
        ),
    }
    return {"stats": stats}
...
def _apply_normalization_np(features, norm_config):
    stats = norm_config["stats"]
    mean = stats["mean"].astype(np.float32)
    denominator = stats["denominator"].astype(np.float32)
    return ((features - mean) / denominator).astype(np.float32)


normalization_config = _build_normalization_config(feat_mean_full, feat_std_full)
normalization_stat_bytes = sum(
    value.nbytes for value in normalization_config["stats"].values()
)
print(
    f"Normalization stats: shared across radars "
    f"({normalization_stat_bytes / 1024:.1f} KiB as float16 stats)"
)

train_features = _apply_normalization_np(train_features_raw, normalization_config)
val_features = _apply_normalization_np(val_features_raw, normalization_config)
test_features = _apply_normalization_np(test_features_raw, normalization_config)
...
def _charbonnier_xy_loss(pred_xy, matched_gt_xy, matched_mask, weights):
...
def _compute_hungarian_loss(xy_true, mask_true, predictions, weights):
    ...
    xy_loss = _charbonnier_xy_loss(pred_xy, matched_gt_xy, matched_mask, w)
...
def _load_tflite_schema():
    schema_path = os.path.join(
        os.path.dirname(tf.__file__), "lite", "python", "schema_py_generated.py"
    )
...
def _normalization_metadata_bytes():
    payload = {
        "schema_version": np.array(2, dtype=np.int32),
        "normalization_stats_mode": np.array("share_radars"),
        "std_epsilon": np.array(STD_EPSILON, dtype=np.float32),
        "thresholds": np.asarray(best_thresholds, dtype=np.float16),
    }
    for name, value in normalization_config["stats"].items():
        payload[name] = value.astype(np.float16)
...
def _add_normalization_metadata_to_tflite(tflite_model):
    schema = _load_tflite_schema()
    model_obj = schema.ModelT.InitFromPackedBuf(tflite_model, 0)
    model_obj.buffers = list(model_obj.buffers or [])
    model_obj.metadata = list(model_obj.metadata or [])

    metadata_buffer = schema.BufferT()
    metadata_payload = _normalization_metadata_bytes()
    metadata_buffer.data = bytearray(metadata_payload)
    model_obj.buffers.append(metadata_buffer)
    buffer_idx = len(model_obj.buffers) - 1

    metadata_entry = None
    model_obj.metadata = [
        entry for entry in model_obj.metadata if entry.name != b"EEAI_NORM_STATS"
    ]
    for entry in model_obj.metadata:
        if entry.name == TFLITE_MODEL_METADATA_NAME:
            metadata_entry = entry
            break
    if metadata_entry is None:
        metadata_entry = schema.MetadataT()
        metadata_entry.name = TFLITE_MODEL_METADATA_NAME
        model_obj.metadata.append(metadata_entry)
    metadata_entry.buffer = buffer_idx
    ...
    print(
        f"Embedded {len(metadata_payload) / 1024:.1f} KiB model metadata "
        f"as {TFLITE_MODEL_METADATA_NAME.decode()}"
    )
    return output
...
def _predict_tflite_arrays(tflite_model, features):
    ...
    for idx in range(len(features)):
        ...
        for o in xy_out:
            ...
            if np.issubdtype(raw.dtype, np.floating) or scale == 0:
                sample_xy.append(raw.astype(np.float32))
            else:
                sample_xy.append((raw.astype(np.float32) - zp) * scale)
        for o in pr_out:
            raw = interpreter.get_tensor(o["index"]).flatten()[0]
            scale, zp = o["quantization"]
            if np.issubdtype(np.asarray(raw).dtype, np.floating) or scale == 0:
                sample_pr.append(float(raw))
            else:
...
print(
    ...
    f"(lr={LEARNING_RATE}, batch={BATCH_SIZE}, schedule=cosine, warmup={WARMUP_EPOCHS})"
)
print(
    f"FFT bins kept: [{fft_bin_lo}, {fft_bin_hi}]  freq kept: [{fft_freq_lo}, {fft_freq_hi}]"
)
print(
    f"Location loss: charbonnier (eps={LOCATION_CHARBONNIER_EPS})"
)
print(
...
for epoch in range(EPOCHS):
    ...
    if (epoch + 1) % TOPK_PTQ_EVAL_EVERY == 0 or (epoch + 1) == EPOCHS:
...
if not topk_candidates:
    raise RuntimeError("Top-K PTQ selection enabled but no checkpoints were saved")
...
hp_str = (
    ...
    f"BACKGROUND=ema EMA_ALPHA={EMA_ALPHA} "
    f"WEIGHT_DECAY={WEIGHT_DECAY} DROPOUT={DROPOUT} "
    f"WARMUP_EPOCHS={WARMUP_EPOCHS} "
    f"MODEL=cplx_fft_dual_encoder_tokens D_ATTN=32 N_HEADS=4 "
    f"COMPLEX_CONV=[32,64,128] "
    f"AMPLITUDE_CONV=[16,32,64] "
    f"CONV_KERNELS=[3,3,3] "
    f"DENSE_DIM=128 HEAD_DIM=32 POS_EMBED=True "
    f"FFT_BIN_RANGE={FFT_BIN_RANGE} FFT_FREQ_RANGE={FFT_FREQ_RANGE} "
    f"NORM=batch ATTENTION_BLOCKS=1 "
    f"POOLING=topk_soft_attn ENCODER_TOPK={ENCODER_TOPK} "
    ...
    f"PTQ_SELECTION=topk TOPK_PTQ_K={TOPK_PTQ_K} "
    ...
    f"ANTENNA_DROPOUT={ANTENNA_DROPOUT} "
    f"MASK_LOSS_WEIGHT={MASK_LOSS_WEIGHT} "
    f"NORMALIZATION_STATS=share_radars"
)
...
tflite_model = _add_normalization_metadata_to_tflite(tflite_model)
...
for idx in range(len(test_features)):
    ...
    for o in xy_out:
        ...
        scale, zp = o["quantization"]
        if np.issubdtype(raw.dtype, np.floating) or scale == 0:
            tflite_xy_all.append(raw.astype(np.float32))
        else:
    ...
    for o in pr_out:
        ...
        scale, zp = o["quantization"]
        if np.issubdtype(np.asarray(raw).dtype, np.floating) or scale == 0:
            tflite_pr_all.append(float(raw))
        else:
...
model_params = {
    ...
    "background_mode": "ema",
    "ema_alpha": EMA_ALPHA,
    "normalization_in_tflite_metadata": np.array(True),
    "normalization_metadata_name": TFLITE_MODEL_METADATA_NAME.decode(),
    "normalization_stats_mode": "share_radars",
}
