...
COUNT_LOSS_WEIGHT = float(os.environ.get("COUNT_LOSS_WEIGHT", 0.2))
...
encoder_count_logits = tf.keras.layers.Dense(5, name="encoder_count_logits")(
    tf.keras.layers.Reshape((N_RADARS * last_conv_dim,))(x)
)
...
outputs.append(encoder_count_logits)
...
def _count_aux_loss(mask_true, predictions, weights):
    labels = tf.cast(tf.reduce_sum(mask_true, axis=1), tf.int32)
...
def compute_total_loss(xy_true, mask_true, predictions, weights):
    loc_loss, xy_l, mask_l = compute_loss(xy_true, mask_true, predictions, weights)
    count_l = _count_aux_loss(mask_true, predictions, weights)
    return loc_loss + COUNT_LOSS_WEIGHT * count_l, xy_l, mask_l, count_l
...
def train_step(x, xy_true, mask_true, weights):
    with tf.GradientTape() as tape:
        ...
        loss, xy_l, mask_l, count_l = compute_total_loss(
            xy_true, mask_true, predictions, weights
        )
    grads = tape.gradient(loss, model.trainable_variables)
    grad_norm = tf.linalg.global_norm(grads)
    optimizer.apply_gradients(zip(grads, model.trainable_variables))
    return loss, xy_l, mask_l, count_l, grad_norm
...
for epoch in range(EPOCHS):
    train_losses, count_losses, grad_norms = [], [], []
    for x_batch, y_batch in train_ds:
        loss, _, _, count_l, gn = train_step(
            x_batch, y_batch["xy"], y_batch["mask"], y_batch["weight"]
        )
        train_losses.append(loss.numpy())
        count_losses.append(count_l.numpy())
    ...
    print(
        ...
        f"count_loss={np.mean(count_losses):.4f}  "
...
hp_str = (
    ...
    f"MODEL=cplx_fft_count_aux_loss D_ATTN={D_ATTN} N_HEADS={N_HEADS} "
...
inference_model = tf.keras.Model(
    inputs=model.inputs, outputs=model.outputs[:8], name="localization_model"
)
...
inference_model.save(os.path.join(model_dir, "model.keras"))
...
for idx in range(len(test_features)):
    ...
    keras_out = inference_model(sample, training=False)
