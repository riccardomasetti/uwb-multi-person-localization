...
x = tf.keras.layers.Lambda(lambda t: tf.reshape(t, [-1, N_RADARS, last_conv_dim]))(x)
...
if POS_EMBED:
    radar_pe = tf.Variable(
        tf.random.normal([1, N_RADARS, d_model], stddev=0.02),
        trainable=True,
...
x = tf.keras.layers.Reshape((N_RADARS * d_model,), name="attention_backbone_features")(x)
backbone_count_logits = tf.keras.layers.Dense(5, name="backbone_count_logits")(x)
backbone_count_model = tf.keras.Model(
    inputs=inp, outputs=backbone_count_logits, name="backbone_count_pretrain_model"
)
...
# ==========================================================================
# Encoder + attention count pretraining
# ==========================================================================
...
def _count_labels(mask_true):
    return tf.cast(tf.reduce_sum(mask_true, axis=1), tf.int32)


def _count_pretrain_loss(mask_true, count_logits, weights):
...
def count_pretrain_step(x, mask_true, weights):
    with tf.GradientTape() as tape:
        logits = backbone_count_model(x, training=True)
        loss = _count_pretrain_loss(mask_true, logits, weights)
    grads = tape.gradient(loss, backbone_count_model.trainable_variables)
    grad_norm = tf.linalg.global_norm(grads)
    count_pretrain_optimizer.apply_gradients(
        zip(grads, backbone_count_model.trainable_variables)
    )
...
def eval_count_pretrain(dataset):
    ...
    for x_batch, y_batch in dataset:
        logits = backbone_count_model(x_batch, training=False)
        loss = _count_pretrain_loss(y_batch["mask"], logits, y_batch["weight"])
        pred = tf.argmax(logits, axis=-1, output_type=tf.int32)
        acc = tf.reduce_mean(
...
if COUNT_PRETRAIN_EPOCHS > 0:
    print(
        f"\nPretraining encoder + attention count head for {COUNT_PRETRAIN_EPOCHS} epochs "
        f"(lr={COUNT_PRETRAIN_LR})"
    )
    best_backbone_weights = None
    ...
    for epoch in range(COUNT_PRETRAIN_EPOCHS):
        ...
        if val_count_loss < best_count_val:
            ...
            best_backbone_weights = backbone_count_model.get_weights()
    if best_backbone_weights is not None:
        backbone_count_model.set_weights(best_backbone_weights)
        print(f"Restored best pretrained backbone (val_count_loss={best_count_val:.4f})")
...
hp_str = (
    ...
    f"MODEL=cplx_fft_backbone_count_pretrain D_ATTN={D_ATTN} N_HEADS={N_HEADS} "
