...
COUNT_PRETRAIN_EPOCHS = int(os.environ.get("COUNT_PRETRAIN_EPOCHS", EPOCHS))
COUNT_PRETRAIN_LR = float(os.environ.get("COUNT_PRETRAIN_LR", LEARNING_RATE))
...
def build_conv_encoder():
    enc_inp = tf.keras.Input(
        shape=(n_bins_kept, n_freq_kept, N_ANTENNAS * n_channels),
        name="single_radar_fft",
    )
    z = enc_inp
    for i, (f, k) in enumerate(zip(conv_filters, conv_kernels)):
        z = tf.keras.layers.Conv2D(
            f, kernel_size=(k, k), padding="same", name=f"enc_conv_{i}"
        )(z)
        z = norm()(z)
        z = tf.keras.layers.ReLU(name=f"enc_relu_{i}")(z)
        z = tf.keras.layers.MaxPool2D(pool_size=(2, 2), name=f"enc_pool_{i}")(z)
    z = tf.keras.layers.GlobalAveragePooling2D(name="enc_gap")(z)
    return tf.keras.Model(enc_inp, z, name="conv_encoder")


conv_encoder = build_conv_encoder()
...
x = conv_encoder(x)
radar_encoder_features = tf.keras.layers.Lambda(
    lambda t: tf.reshape(t, [-1, N_RADARS, last_conv_dim]),
    name="radar_encoder_features",
)(x)

count_logits_per_radar = tf.keras.layers.Dense(
    5, name="single_radar_count_logits"
)(radar_encoder_features)
encoder_count_model = tf.keras.Model(
    inputs=inp, outputs=count_logits_per_radar, name="encoder_count_pretrain_model"
)

x = tf.keras.layers.Dense(d_model, name="radar_embedding")(radar_encoder_features)

if POS_EMBED:
    radar_pe_layer = tf.keras.layers.Embedding(
        input_dim=N_RADARS,
        output_dim=d_model,
        embeddings_initializer=tf.keras.initializers.RandomNormal(stddev=0.02),
        name="radar_pos_embed",
    )
    radar_positions = tf.keras.layers.Lambda(
        lambda t: tf.tile(tf.range(N_RADARS)[None, :], [tf.shape(t)[0], 1])
    )
    x = tf.keras.layers.Add()([x, radar_pe_layer(radar_positions(x))])
...
# ==========================================================================
# Encoder count pretraining (single-radar linear probe)
# ==========================================================================
count_pretrain_optimizer = tf.keras.optimizers.Adam(
    COUNT_PRETRAIN_LR, weight_decay=WEIGHT_DECAY
)


def _count_labels(mask_true):
    counts = tf.cast(tf.reduce_sum(mask_true, axis=1), tf.int32)
    return tf.tile(counts[:, None], [1, N_RADARS])


def _count_probe_loss(mask_true, count_logits, weights):
    labels = _count_labels(mask_true)
    per_radar = tf.keras.losses.sparse_categorical_crossentropy(
        labels, count_logits, from_logits=True
    )
    w = tf.cast(weights[:, None], per_radar.dtype)
    return tf.reduce_sum(per_radar * w) / (tf.reduce_sum(w) * N_RADARS + 1e-8)


@tf.function
def count_pretrain_step(x, mask_true, weights):
    with tf.GradientTape() as tape:
        logits = encoder_count_model(x, training=True)
        loss = _count_probe_loss(mask_true, logits, weights)
    grads = tape.gradient(loss, encoder_count_model.trainable_variables)
    grad_norm = tf.linalg.global_norm(grads)
    count_pretrain_optimizer.apply_gradients(
        zip(grads, encoder_count_model.trainable_variables)
    )
    pred = tf.argmax(logits, axis=-1, output_type=tf.int32)
    acc = tf.reduce_mean(tf.cast(pred == _count_labels(mask_true), tf.float32))
    return loss, acc, grad_norm


def eval_count_pretrain(dataset):
    losses, accs = [], []
    for x_batch, y_batch in dataset:
        logits = encoder_count_model(x_batch, training=False)
        loss = _count_probe_loss(y_batch["mask"], logits, y_batch["weight"])
        pred = tf.argmax(logits, axis=-1, output_type=tf.int32)
        acc = tf.reduce_mean(
            tf.cast(pred == _count_labels(y_batch["mask"]), tf.float32)
        )
...
if COUNT_PRETRAIN_EPOCHS > 0:
    print(
        f"\nPretraining convolutional encoder count probe for {COUNT_PRETRAIN_EPOCHS} epochs "
        f"(lr={COUNT_PRETRAIN_LR}, single-radar logits)"
    )
    best_encoder_weights = None
    best_count_val = float("inf")
    for epoch in range(COUNT_PRETRAIN_EPOCHS):
        losses, accs, grad_norms = [], [], []
        for x_batch, y_batch in train_ds:
            loss, acc, gn = count_pretrain_step(
                x_batch, y_batch["mask"], y_batch["weight"]
            )
            losses.append(loss.numpy())
            accs.append(acc.numpy())
            grad_norms.append(gn.numpy())
        val_count_loss, val_count_acc = eval_count_pretrain(val_ds)
        print(
            f"Count pretrain {epoch + 1:3d}/{COUNT_PRETRAIN_EPOCHS}  "
            f"train_loss={np.mean(losses):.4f}  "
            f"train_acc={np.mean(accs) * 100:.1f}%  "
            f"val_loss={val_count_loss:.4f}  "
            f"val_acc={val_count_acc * 100:.1f}%  "
            f"grad_norm={np.mean(grad_norms):.4f}"
        )
        if val_count_loss < best_count_val:
            best_count_val = val_count_loss
            best_encoder_weights = conv_encoder.get_weights()
    if best_encoder_weights is not None:
        conv_encoder.set_weights(best_encoder_weights)
        print(f"Restored best pretrained encoder (val_count_loss={best_count_val:.4f})")
...
def _compute_hungarian_loss(xy_true, mask_true, predictions, weights):
    ...
    return xy_loss + MASK_LOSS_WEIGHT * mask_loss, xy_loss, mask_loss
...
hp_str = (
    ...
    f"MODEL=cplx_fft_encoder_count_pretrain D_ATTN={D_ATTN} N_HEADS={N_HEADS} "
    ...
    f"COUNT_PRETRAIN_EPOCHS={COUNT_PRETRAIN_EPOCHS} "
    f"COUNT_PRETRAIN_LR={COUNT_PRETRAIN_LR}"
)
