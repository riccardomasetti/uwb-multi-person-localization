...
# ==========================================================================
# Model definition: per-radar Conv1D + attention fusion + 4 heads
# ==========================================================================
N_RADARS = 6
N_ANTENNAS = 3
N_BINS = 120
PER_RADAR_DIM = N_ANTENNAS * N_BINS * 2
...
x = tf.keras.layers.Reshape((SUB_WINDOW_SIZE, N_RADARS, PER_RADAR_DIM))(inp)
x = tf.keras.layers.Permute((2, 1, 3))(x)
x = tf.keras.layers.Lambda(
    lambda t: tf.reshape(t, [-1, SUB_WINDOW_SIZE, PER_RADAR_DIM])
)(x)
...
if POS_EMBED:
    radar_pe = tf.Variable(
        tf.random.normal([1, N_RADARS, last_conv_dim], stddev=0.02),
        trainable=True,
...
hp_str = (
    ...
    f"MODEL=attn D_ATTN={D_ATTN} N_HEADS={N_HEADS} "
