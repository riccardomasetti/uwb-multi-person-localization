...
# ==========================================================================
# Model definition: per-radar Conv2D (on FFT spectrogram) + attention + 4 heads
# ==========================================================================
N_RADARS = 6
N_ANTENNAS = 3
N_BINS = 120
...
x = tf.keras.layers.Permute((1, 3, 4, 2))(inp)
x = tf.keras.layers.Lambda(
    lambda t: tf.reshape(t, [-1, n_bins_kept, n_freq_kept, N_ANTENNAS])
)(x)
...
for fpath in files:
    ...
    for m_idx in range(MACRO_WINDOW_SPLIT):
        ...
        amplitude = np.sqrt(iq_m[..., 0] ** 2 + iq_m[..., 1] ** 2)
...
if BASELINE_ZERO_PEOPLE:
    ...
    if len(zero_train_idx) == 0:
        ...
        baseline = bl_med.astype(np.float32)
...
hp_str = (
    ...
    f"MODEL=attn_fft D_ATTN={D_ATTN} N_HEADS={N_HEADS} "
...
csv_path = os.path.join(os.path.dirname(__file__) or ".", "attn_fft_results.csv")
...
q_csv_path = os.path.join(
    os.path.dirname(__file__) or ".", "attn_fft_quant_results.csv"
)
q_header = [
    "experiment_name",
    "qt_type",
