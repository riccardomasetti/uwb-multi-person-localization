...
def _topk_for_pooling():
    if ENCODER_POOLING == "topk8_soft_attn":
        return 8
    return ENCODER_TOPK


def _attention_temperature_for_pooling():
    if ENCODER_POOLING == "soft_attn_temp4":
        return 4.0
    return ENCODER_ATTN_TEMPERATURE


def _encoder_pool_dim(conv_dim):
    if ENCODER_POOLING in ("topk_soft_attn", "topk8_soft_attn"):
        return conv_dim * 2
    return conv_dim
...
def encoder_pool(x):
    if ENCODER_POOLING == "gap":
        return tf.keras.layers.GlobalAveragePooling2D(name="encoder_gap")(x)
    if ENCODER_POOLING in ("soft_attn", "soft_attn_temp4"):
        return SoftAttentionPooling2D(
            _attention_temperature_for_pooling(), name="encoder_soft_attn"
        )(x)
    if ENCODER_POOLING in ("topk_soft_attn", "topk8_soft_attn"):
        topk = TopKAveragePooling2D(_topk_for_pooling(), name="encoder_topk")(x)
        attn = SoftAttentionPooling2D(
            _attention_temperature_for_pooling(), name="encoder_soft_attn"
        )(x)
        return tf.keras.layers.Concatenate(name="encoder_topk_soft_attn")([topk, attn])
    raise ValueError(f"Unsupported ENCODER_POOLING: {ENCODER_POOLING}")
...
pool_dim = _encoder_pool_dim(last_conv_dim)

x = encoder_pool(x)
x = tf.keras.layers.Lambda(lambda t: tf.reshape(t, [-1, N_RADARS, pool_dim]))(x)
...
hp_str = (
    ...
    f"MODEL=cplx_fft D_ATTN={D_ATTN} N_HEADS={N_HEADS} "
