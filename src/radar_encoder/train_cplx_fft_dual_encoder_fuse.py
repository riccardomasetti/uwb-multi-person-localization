...
for i, (cplx_f, amp_f, k) in enumerate(
    ...
    x_cplx = tf.keras.layers.Conv2D(
        cplx_f, kernel_size=(k, k), padding="same", name=f"complex_conv2d_{i}"
    )(x_cplx)
    x_cplx = norm()(x_cplx)
    x_cplx = tf.keras.layers.ReLU()(x_cplx)
    x_cplx = tf.keras.layers.MaxPool2D(pool_size=(2, 2))(x_cplx)

    x_amp = tf.keras.layers.Conv2D(
        amp_f, kernel_size=(k, k), padding="same", name=f"amplitude_conv2d_{i}"
    )(x_amp)
    x_amp = norm()(x_amp)
    x_amp = tf.keras.layers.ReLU()(x_amp)
    x_amp = tf.keras.layers.MaxPool2D(pool_size=(2, 2))(x_amp)

complex_last_conv_dim = complex_conv_filters[-1]
amplitude_last_conv_dim = amplitude_conv_filters[-1]

x_cplx = tf.keras.layers.GlobalAveragePooling2D()(x_cplx)
x_cplx = tf.keras.layers.Lambda(
    lambda t: tf.reshape(t, [-1, N_RADARS, complex_last_conv_dim])
)(x_cplx)
x_amp = tf.keras.layers.GlobalAveragePooling2D()(x_amp)
x_amp = tf.keras.layers.Lambda(
    lambda t: tf.reshape(t, [-1, N_RADARS, amplitude_last_conv_dim])
)(x_amp)
...
hp_str = (
    ...
    f"MODEL=cplx_fft_dual_encoder_fuse D_ATTN={D_ATTN} N_HEADS={N_HEADS} "
