...
# ==========================================================================
# Model definition: per-antenna Conv1D + antenna attention + radar attention + 4 heads
# ==========================================================================
N_RADARS = 6
N_ANTENNAS = 3
N_BINS = 120
...
x = tf.keras.layers.Lambda(
    lambda t: tf.reshape(t, [-1, SUB_WINDOW_SIZE, N_BINS * 2])
)(x)

for i, (f, k) in enumerate(zip(conv_filters, conv_kernels)):
    x = tf.keras.layers.Conv1D(f, kernel_size=k, padding="same")(x)
    ...
    for x_input, y_batch in dataset:
        ...
        if SLIDING_WINDOW:
            ...
            for pos in positions:
                ...
                _accumulate_metrics(
                    pred_xy, mask_pred, xy_t, mask_t, all_xy_err, all_count_correct
                )
        else:
            ...
            mask_pred = expit(pred_pr) > 0.5
            _accumulate_metrics(
                pred_xy, mask_pred, xy_true, mask_true, all_xy_err, all_count_correct
            )
...
hp_str = (
    ...
    f"MODEL=attn_ant D_ATTN={D_ATTN} N_HEADS={N_HEADS} "
...
csv_path = os.path.join(os.path.dirname(__file__) or ".", "attn_results.csv")
