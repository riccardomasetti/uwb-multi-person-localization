...
# ==========================================================================
# Model definition: per-antenna Conv2D (on FFT spectrogram) + antenna attention + radar attention + 4 heads
# ==========================================================================
N_RADARS = 6
N_ANTENNAS = 3
N_BINS = 120
N_FREQ = SUB_WINDOW_SIZE // 2 + 1
...
inp = tf.keras.Input(
    shape=(N_RADARS, N_ANTENNAS, n_bins_kept, n_freq_kept), name="input"
)

# Flatten batch*radars*antennas so each antenna is processed independently
x = tf.keras.layers.Lambda(
    lambda t: tf.reshape(t, [-1, n_bins_kept, n_freq_kept, 1])
)(inp)
...
# Reshape back to (B, 6, 3, last_conv_dim)
x = tf.keras.layers.GlobalAveragePooling2D()(x)
x = tf.keras.layers.Lambda(
    lambda t: tf.reshape(t, [-1, N_RADARS, N_ANTENNAS, last_conv_dim])
)(x)

# Intra-radar antenna attention: process each radar's 3 antennas
x = tf.keras.layers.Lambda(
    lambda t: tf.reshape(t, [-1, N_ANTENNAS, last_conv_dim])
)(x)  # (B*6, 3, last_conv_dim)

x = AntennaAttention(D_ATTN, N_HEADS)(x)  # (B*6, 3, D_ATTN*N_HEADS)

# Concatenate 3 antenna outputs per radar and project down to one radar token
x = tf.keras.layers.Reshape((N_ANTENNAS * D_ATTN * N_HEADS,))(x)
x = tf.keras.layers.Dense(ANTENNA_D_PROJ)(x)  # (B*6, ANTENNA_D_PROJ)

# Reshape back to radar tokens: (B, 6, ANTENNA_D_PROJ)
x = tf.keras.layers.Lambda(
...
x = tf.keras.layers.Reshape((N_RADARS * D_ATTN * N_HEADS,))(x)
...
def _process_window(iq, people_xy, people_mask):
    ...
    fft_vals = np.fft.rfft(amplitude, axis=1)
    # shape: (n_sub, N_FREQ, 6, 3, 120)

    fft_abs = np.abs(fft_vals).astype(np.float64)
    # shape: (n_sub, N_FREQ, 6, 3, 120)

    # Transpose to (n_sub, 6, 3, 120, N_FREQ)
    fft_abs = fft_abs.transpose(0, 2, 3, 4, 1)

    # Slice bin and freq ranges
    fft_abs = fft_abs[
        :, :, :, fft_bin_lo : fft_bin_hi + 1, fft_freq_lo : fft_freq_hi + 1
    ]
    # shape: (n_sub, 6, 3, n_bins_kept, n_freq_kept)

    features = fft_abs.astype(np.float32)
...
# ==========================================================================
# Standardization (mean/std per radar, antenna, bin, freq — computed on train set)
# ==========================================================================
...
_sample_spec = (
    tf.TensorSpec(
        shape=(N_RADARS, N_ANTENNAS, n_bins_kept, n_freq_kept), dtype=tf.float32
    ),
    {
